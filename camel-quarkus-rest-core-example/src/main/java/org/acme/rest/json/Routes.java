...
public class Routes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        /*
         * // create a route template with the given name routeTemplate("myTemplate") // here we define the required
        ...
        /*
         * TemplatedRouteBuilder.builder(getContext(), "myTemplate").parameter("name", "two") .parameter("greeting",
         * "Bonjour").parameter("myPeriod", "5s").add();
         */

        rest("/fruits").get().to("direct:getFruits")

                .post().type(Fruit.class).to("direct:addFruit");

        rest("/legumes").get().to("direct:getLegumes");

        from("direct:getFruits").setBody().constant(fruits);

        from("direct:addFruit").process().body(Fruit.class, fruits::add).setBody().constant(fruits);

        from("direct:getLegumes").setBody().constant(legumes);
